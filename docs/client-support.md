...
SlateFS currently serves NFSv3 and 9P2000.L over TCP. The daemon does not yet
provide a virtio device; QEMU `virtio-9p` is QEMU's own host-filesystem server,
not a transport to an arbitrary TCP 9P server.
