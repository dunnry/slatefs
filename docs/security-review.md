...
| Version commit authenticity | Optional detached Ed25519 attestations bind the stable logical repository UUID, commit ID, key ID, public key, and timestamp without coupling signatures to local tenant/volume placement. Protected branches may require a configurable N-of-M quorum of exact trusted keys before fast-forward publication; private keys remain client-side. | `detached_commit_attestations_are_optional_immutable_and_verified`; live admin versioning integration test. |
