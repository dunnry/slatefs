...
impl Decodable for DirEntryData {
    fn decode<R: ReadBytesExt>(r: &mut R) -> Result<Self> {
        // [slatefs patch] The Rreaddir prefix is the BYTE length of the
        // entry blob (which is what `encode` writes via `self.size()`),
        // not an entry count. Decoding it as a count over-read the stream;
        // consume exactly `count` bytes of entries instead.
        let count: u32 = Decodable::decode(r)?;
        let blob = read_exact(r, count as usize)?;
        let mut cursor = ::std::io::Cursor::new(blob);
        let mut data: Vec<DirEntry> = Vec::new();
        while (cursor.position() as usize) < cursor.get_ref().len() {
            data.push(Decodable::decode(&mut cursor)?);
        }
