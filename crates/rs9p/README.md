...
---
**Vendored fork notice (slatefs):** rs9p 0.13.0 (BSD-3-Clause,
https://github.com/rs9p/rs9p) vendored with minimal patches marked
`[slatefs patch]`:
1. portable statvfs conversion (macOS nix returns u32 fields; the crate
   only compiled on Linux);
2. `DirEntryData::decode` treated the Rreaddir length prefix as an entry
   COUNT; the wire format (and the crate's own `encode`) use the BYTE
   length of the entry blob. Server paths never decode R-messages, so the
   bug only affects clients built on the codec.
Intended for upstream PRs; original LICENSE retained.
