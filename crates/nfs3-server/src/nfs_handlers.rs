    ...
    T: NfsFileSystem,
{
    ...
    // [slatefs patch] expose the caller's AUTH_UNIX identity to the
    // filesystem for the duration of this request (see request_auth.rs).
    let auth = context.auth.clone();
    crate::request_auth::CURRENT_AUTH
        .scope(auth, async move {
            match proc {
                ...
                // [slatefs patch] previously PROC_UNAVAIL
                NFSPROC3_LINK => handle(context, message, nfsproc3_link).await,
                NFSPROC3_MKNOD => handle(context, message, nfsproc3_mknod).await,
            }
        })
        .await
}
    ...
    T: NfsFileSystem,
{
    ...
    // [slatefs patch] let the filesystem mask the bits per the caller's
    // identity; the trait default grants everything requested (old behavior).
    let access = match context.vfs.access(&id, access).await {
        Ok(allowed) => allowed,
        Err(stat) => {
            error!("access error {xid} --> {stat}");
            return ACCESS3res::Err((stat, ACCESS3resfail { obj_attributes }));
        }
    };
    ...
    T: NfsFileSystem,
{
    let handle = args.fsroot;
    let id = fh_to_id!(context, &handle);
    // [slatefs patch] delegate to the filesystem so real capacity/usage can
    // be reported; the trait default preserves the old hardcoded values.
    match context.vfs.fsstat(&id).await {
        ...
        Err(stat) => {
            ...
            FSSTAT3res::Err((stat, FSSTAT3resfail::default()))
        }
    }
}
...
// [slatefs patch] LINK3resfail/MKNOD3resfail don't derive Default upstream.
fn link3_resfail() -> LINK3resfail {
    LINK3resfail {
...
fn mknod3_resfail() -> MKNOD3resfail {
    MKNOD3resfail {
        dir_wcc: wcc_data::default(),
    }
}

// [slatefs patch] LINK handler (RFC 1813 §3.3.15); previously PROC_UNAVAIL.
async fn nfsproc3_link<T>(context: RPCContext<T>, xid: u32, args: LINK3args<'_>) -> LINK3res
where
    T: NfsFileSystem,
{
    if !matches!(context.vfs.capabilities(), VFSCapabilities::ReadWrite) {
        warn!("No write capabilities.");
        return LINK3res::Err((nfsstat3::NFS3ERR_ROFS, link3_resfail()));
    }

    let fileid = match context.file_handle_converter.fh_from_nfs(&args.file) {
        Ok(id) => id,
        Err(stat) => return LINK3res::Err((stat, link3_resfail())),
    };
    let dirid = match context.file_handle_converter.fh_from_nfs(&args.link.dir) {
        Ok(id) => id,
        Err(stat) => return LINK3res::Err((stat, link3_resfail())),
    };
    ...
    let before = match get_wcc_attr(&context, &dirid).await {
        ...
        Err(stat) => {
            warn!("Cannot stat directory {xid} --> {stat}");
            return LINK3res::Err((stat, link3_resfail()));
        }
    };

    let result = context.vfs.link(&fileid, &dirid, &args.link.name).await;
    let after = nfs_option_from_result(context.vfs.getattr(&dirid).await);
    let linkdir_wcc = wcc_data { before, after };

    match result {
        Ok(fattr) => {
            debug!("link success {xid} --> {fattr:?}");
            LINK3res::Ok(LINK3resok {
                file_attributes: post_op_attr::Some(fattr),
                linkdir_wcc,
            })
        }
        Err(stat) => {
            error!("link error {xid} --> {stat}");
            let file_attributes = nfs_option_from_result(context.vfs.getattr(&fileid).await);
            LINK3res::Err((
                stat,
                LINK3resfail {
                    file_attributes,
...
// [slatefs patch] MKNOD handler (RFC 1813 §3.3.11); previously PROC_UNAVAIL.
async fn nfsproc3_mknod<T>(context: RPCContext<T>, xid: u32, args: MKNOD3args<'_>) -> MKNOD3res
where
    T: NfsFileSystem,
{
    if !matches!(context.vfs.capabilities(), VFSCapabilities::ReadWrite) {
        warn!("No write capabilities.");
        return MKNOD3res::Err((nfsstat3::NFS3ERR_ROFS, mknod3_resfail()));
    }

    let dirid = match context.file_handle_converter.fh_from_nfs(&args.where_.dir) {
        Ok(id) => id,
        Err(stat) => return MKNOD3res::Err((stat, mknod3_resfail())),
    };
    ...
    let before = match get_wcc_attr(&context, &dirid).await {
        ...
        Err(stat) => {
            warn!("Cannot stat directory {xid} --> {stat}");
            return MKNOD3res::Err((stat, mknod3_resfail()));
        }
    };

    let result = context
        .vfs
        .mknod(&dirid, &args.where_.name, &args.what)
        .await;
    let after = nfs_option_from_result(context.vfs.getattr(&dirid).await);
    let dir_wcc = wcc_data { before, after };
    ...
    match result {
        Ok((fid, fattr)) => {
            debug!("mknod success {xid} --> {fid:?}, {fattr:?}");
            MKNOD3res::Ok(MKNOD3resok {
                obj: post_op_fh3::Some(context.file_handle_converter.fh_to_nfs(&fid)),
                obj_attributes: post_op_attr::Some(fattr),
                dir_wcc,
            })
        ...
        Err(stat) => {
            ...
            MKNOD3res::Err((stat, MKNOD3resfail { dir_wcc }))
        }
    }
}
